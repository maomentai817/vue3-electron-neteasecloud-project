<script setup>
import { useMusicStore, useUserStore, useGlobalStore } from '@/stores'
...
// 音量控制
let previousVolume = userStore.volume
...
const volumeHandler = (target) => {
  ...
  if (target) {
    previousVolume = userStore.volume
    model.value = 0
  } else {
    model.value = previousVolume * 100
  }
}

const change = (val) => {
  model.value = val
  if (val) userStore.volume = val / 100
}

const emit = defineEmits(['input', 'volume', 'open'])
const input = (val) => {
  emit('input', val / 100)
}
...
<template>
  <div class="detail-right f-s fs-13" v-if="musicStore.songInfo?.id">
    <div class="timer f-b m-r-10% w-105 color-#fff">
      <span class="current w-50 flex-shrink-0 text-center">{{
...
:deep(.el-slider__bar) {
  background-color: v-bind('globalStore.color') !important;
